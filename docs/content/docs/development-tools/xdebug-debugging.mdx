...
<Callout type="info">
  - **develop,debug** - Full debugging capabilities enabled - **trigger** - Debugging starts only
  when triggered (via browser extension or URL parameter) - **9003** - Standard XDebug 3.x port
</Callout>
...
<Callout type="info">
  XDebug is configured with `start_with_request = trigger`, which means debugging only starts when
  triggered - giving you full control over when to debug.
</Callout>
...
<Callout type="tip">
  Instead of adding `echo` or `var_dump`, use logpoints. Right-click and select "Add Logpoint" to
  log messages without modifying code.
</Callout>
...
<Callout type="success">
  XDebug is ready to use in every project - no setup required. Open in VS Code, press F5, and start
  debugging.
</Callout>
