...
<Callout type="info">
  Volume sync is bidirectional - changes sync from local filesystem to named Docker volume and vice
  versa.
</Callout>
...
<Callout type="tip">
  Work in VS Code DevContainer for speed → Sync to local when you need to commit or backup → Sync to
  volume when editing locally.
</Callout>
